...
namespace Presentation.Controllers
{
    ...
    public class ImageGroupController : ControllerBase
    {
        ...
        [HttpGet("GetAll")]
        [AuthorizePermission("ImageGroup", "Read")]
        public async Task<IActionResult> GetAllImageGroupsAsync()
        {
            ...
            return Ok(
                new GetAllRequest<IEnumerable<ImageGroupDto>>(imageGroup, 1, "ImageGroup", _logger)
            );
        }
        ...
        {
            var imageGroup = await _manager.ImageGroupService.SortImageGroupAsync(
                id,
                newSort,
                false
            );
        ...
        [HttpPost("Create")]
        [AuthorizePermission("ImageGroup", "Write")]
        public async Task<IActionResult> CreateImageGroupAsync(
            [FromBody] ImageGroupDtoForInsertion imageGroupDtoForInsertion
        )
        {
            var imageGroup = await _manager.ImageGroupService.CreateImageGroupAsync(
                imageGroupDtoForInsertion
            );
        ...
        [HttpPut("Update")]
        [AuthorizePermission("ImageGroup", "Write")]
        public async Task<IActionResult> UpdateImageGroupAsync(
            [FromBody] ImageGroupDtoForUpdate imageGroupDtoForUpdate
        )
        {
            var imageGroup = await _manager.ImageGroupService.UpdateImageGroupAsync(
                imageGroupDtoForUpdate.ImageGroupID,
                imageGroupDtoForUpdate,
                false
            );
        ...
        [AuthorizePermission("ImageGroup", "Delete")]
