...
namespace Presentation.Controllers
{
    ...
    public class SettingController : ControllerBase
    {
        ...
        [HttpPost("Create")]
        public async Task<IActionResult> CreateSettingAsync(IFormFile? file, [FromForm] SettingDtoForInsertion settingDtoForInsertion)
        {
            var rnd = new Random();
            var imgId = rnd.Next(0, 100000);
            var upload = file != null ? await FileManager.FileUpload(file, imgId, "images") : null;

            settingDtoForInsertion.FileName = file != null ? $"{imgId}_{upload["FilesName"]}" : null;
            settingDtoForInsertion.FilePath = file != null ? $"{upload["FilesPath"]}" : null;
            settingDtoForInsertion.FileFullPath = file != null ? $"{upload["FilesFullPath"]}" : null;
        ...
        [Authorize(AuthenticationSchemes = "Bearer")]
        [HttpPut("Update")]
        public async Task<IActionResult> UpdateSettingAsync(IFormFile? file, [FromForm] SettingDtoForUpdate settingDtoForUpdate)
        {
            var rnd = new Random();
            var imgId = rnd.Next(0, 100000);
            
            var setting = await _manager.SettingService.GetSettingByIdAsync(settingDtoForUpdate.SettingID, false);
            
            var upload = file != null ? await FileManager.FileUpload(file, imgId, "images") : null;
        
            settingDtoForUpdate.FileName = file != null ? $"{imgId}_{upload["FilesName"]}" : setting.FileName;
            settingDtoForUpdate.FilePath = file != null ? $"{upload["FilesPath"]}" : setting.FilePath;
            settingDtoForUpdate.FileFullPath = file != null ? $"{upload["FilesFullPath"]}" : setting.FileFullPath;
            
            var sett = await _manager.SettingService.UpdateSettingAsync(settingDtoForUpdate.SettingID, settingDtoForUpdate, false);
